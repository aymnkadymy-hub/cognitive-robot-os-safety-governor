//! # walker-policy
//!
//! **Walking** policy (MLP 17→64→64→6 with tanh) trained via PPO on **MuJoCo Walker2d physics**,
//! executed `no_std` heap-free `unsafe`-free on seL4. Input: body state (17 dimensions); output:
//! 6 joint commands ∈ [-1,1] (clipped/governed by the guard before actuators).
//!
//! Demonstrates that a reinforcement-learned locomotion policy runs on the verified kernel (body = real robot later).
...
pub fn command(state: &[f32; weights::IN]) -> [f32; weights::OUT] {
    ...
    for v in a.iter_mut() {
        *v = v.clamp(-1.0, 1.0); // Walker2d action space
    }
    a
}
...
mod tests {
    ...
    fn matches_reference_within_epsilon() {
        // SIL: Rust inference matches torch (PPO) on the same weights (before clipping).
